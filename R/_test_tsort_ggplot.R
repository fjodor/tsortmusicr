...
#' @param reorder If TRUE, reorders categories by frequency (ascending)
...
tsm_ggbox <- function(data,
  ...
  # if (reorder) data[{{x}}] <- forcats::fct_rev(forcats::fct_infreq(data[{{x}}]))
  if (reorder) {
    data <- data %>%
      mutate({{x}} := forcats::fct_rev(forcats::fct_infreq({{x}})))
  }
  ...
  plot <- ggplot2::ggplot(data, ggplot2::aes(x = x, y = y, col = artist)) +
